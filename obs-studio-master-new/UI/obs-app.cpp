...
bool OBSApp::SetTheme(std::string name, std::string path)
{
	...
	//path = "../../data/obs-studio/themes/Rachni.qss";
...
bool OBSApp::InitTheme()
{
	...
	const char *themeName =	config_get_string(globalConfig, "General", "CurrentTheme2");

	if (!themeName)
		/* Use deprecated "CurrentTheme" value if available */
		themeName = config_get_string(globalConfig, "General","CurrentTheme");
	if (!themeName)
...
bool OBSApp::OBSInit()
{
	...
	//确保原生小部件的兄弟窗口保持非原生，除非 Qt::WA_NativeWindow 属性特别设置
	...
	*/
	...
	//即控制着当最后一个可视的窗口退出时候，程序是否退出，默认是是的,当前设置为否
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	QGuiApplication::setAttribute(opt_disable_high_dpi_scaling ? Qt::AA_DisableHighDpiScaling: Qt::AA_EnableHighDpiScaling);
#endif
	...
	try {
		...
		if (!multi) {
			...
			QMessageBox mb(QMessageBox::Question,QTStr("AlreadyRunning.Title"),QTStr("AlreadyRunning.Text"), buttons,nullptr);
			mb.setButtonText(QMessageBox::Yes,QTStr("AlreadyRunning.LaunchAnyway"));
...
#endif
	// 解析 C:\Users\Administrator\AppData\Roaming\obs-studio\basic\profiles 目录(包括子目录)下得所有配置文件	
	upgrade_settings();
