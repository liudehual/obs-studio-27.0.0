	  ...
	  showInactive(false)
{
	...
	usePercent->setText("%");
	connect(usePercent, SIGNAL(toggled(bool)), this,SLOT(SetVolumeType(bool)));

	VolumeType volType = (VolumeType)config_get_int(GetGlobalConfig(), "BasicWindow", "AdvAudioVolumeType");
...
void OBSBasicAdvAudio::OBSSourceRemoved(void *param, calldata_t *calldata)
{
	...
	QMetaObject::invokeMethod(reinterpret_cast<OBSBasicAdvAudio *>(param), "SourceRemoved", Q_ARG(OBSSource, source));
}
