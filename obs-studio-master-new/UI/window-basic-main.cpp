...
OBSBasic::OBSBasic(QWidget *parent)	: OBSMainWindow(parent), ui(new Ui::OBSBasic)
{
	...
	statsDock->setFeatures(QDockWidget::DockWidgetClosable |  QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable);
	...
	connect(ui->scenes->itemDelegate(),SIGNAL(closeEditor(QWidget *, QAbstractItemDelegate::EndEditHint)),this,
		SLOT(SceneNameEdited(QWidget *,QAbstractItemDelegate::EndEditHint)));

	cpuUsageInfo = os_cpu_usage_info_start();
	cpuUsageTimer = new QTimer(this);
	connect(cpuUsageTimer.data(), SIGNAL(timeout()), ui->statusbar,SLOT(UpdateCPUUsage()));
	...
	connect(ui->scenes, SIGNAL(scenesReordered()), this,SLOT(ScenesReordered()));
}
...
void OBSBasic::Save(const char *file)
{
	...
	obs_data_t *saveData = GenerateSaveData(sceneOrder, quickTrData, ui->transitionDuration->value(),transitions, scene, curProgramScene, savedProjectorList);
...
void OBSBasic::Load(const char *file)
{
	...
	//调用 sendPostedEvents 函数会立即对前面已经通过 QCoreApplication::postEvent() 排队等待的所有事件立即进行派发(处理)
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	if (config_has_user_value(basicConfig, "SimpleOutput", "EnforceBitrate") && !config_has_user_value(basicConfig, "Stream1","IgnoreRecommended") &&
	...
	config_set_default_string(basicConfig, "Audio", "MonitoringDeviceName",Str("Basic.Settings.Advanced.Audio.MonitoringDevice"".Default"));
...
void OBSBasic::InitOBSCallbacks()
{
	...
	signalHandlers.emplace_back(obs_get_signal_handler(), "source_audio_activate", OBSBasic::SourceAudioActivated, this);
	signalHandlers.emplace_back(obs_get_signal_handler(), "source_audio_deactivate",  OBSBasic::SourceAudioDeactivated, this);
...
{
	vcamButton = new ReplayBufferButton(QTStr("Basic.Main.StartVirtualCam"),this);
	vcamButton->setCheckable(true);
	connect(vcamButton.data(), &QPushButton::clicked, this,&OBSBasic::VCamButtonClicked);
...
void OBSBasic::ResetOutputs()
{
	...
	if (!outputHandler || !outputHandler->Active()) {
		...
		if (outputHandler->replayBuffer) {
			replayBufferButton = new ReplayBufferButton(QTStr("Basic.Main.StartReplayBuffer"), this);
			replayBufferButton->setCheckable(true);
			connect(replayBufferButton.data(),&QPushButton::clicked, this,&OBSBasic::ReplayBufferClicked);

			replayBufferButton->setSizePolicy(QSizePolicy::Ignored,QSizePolicy::Fixed);
			...
			setTabOrder(replayBufferButton,ui->buttonsVLayout->itemAt(3)->widget());
		}
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, 512, "obs-studio/basic/scenes/%s.json",sceneCollection);
	if (ret <= 0)
		throw "Failed to create scene collection file name";

	//savePath == C:\Users\Administrator\AppData\Roaming\obs-studio\basic\scenes\未命名.json
	ret = GetConfigPath(savePath, sizeof(savePath), fileName);
	if (ret <= 0)
		throw "Failed to get scene collection json file path";

	// 配置  C:\Users\Administrator\AppData\Roaming\obs-studio\basic\profiles\未命名\basic.ini 配置文件中的内容
	if (!InitBasicConfig())
	...
	AddExtraModulePaths();
	blog(LOG_INFO, "---------------------------------");
	// 加载所有模块并执行模块load()函数，注册源/过滤器/输出/服务等
	...
	if (!opt_studio_mode) {
		SetPreviewProgramMode(config_get_bool(App()->GlobalConfig(),"BasicWindow","PreviewProgramMode"));
	} else {
...
#undef SET_VISIBILITY
	...
	// 设置源图标
	bool sourceIconsVisible = config_get_bool(GetGlobalConfig(), "BasicWindow", "ShowSourceIcons");
	ui->toggleSourceIcons->setChecked(sourceIconsVisible);

	//设置源工具条，不同源具有不同的工具条
	bool contextVisible = config_get_bool(App()->GlobalConfig(), "BasicWindow", "ShowContextToolbars");
	ui->toggleContextBar->setChecked(contextVisible);
	ui->contextContainer->setVisible(contextVisible);
	if (contextVisible)
		UpdateContextBar(true);

	// 初始化程序，加载配置文件中配置的信息，场景/源/输出/服务/编码器等功能
	{
		// 配置 C:\Users\Administrator\AppData\Roaming\obs-studio\basic\scenes\未命名.json 配置文件中的内容
	...
	multiviewProjectorMenu = new QMenu(QTStr("MultiviewProjector")); // 视图菜单 多视图(全屏)
	...
	ui->viewMenu->addAction(QTStr("MultiviewWindowed"), this,SLOT(OpenMultiviewWindow())); // 视图菜单 多视图(窗口)
...
void OBSBasic::OnFirstLoad()
{
	...
	bool showLogViewerOnStartup = config_get_bool(	App()->GlobalConfig(), "LogViewer", "ShowLogStartup");
...
#define MAKE_CALLBACK(pred, method, log_action)                            \
	...
	contextBarHotkeys = obs_hotkey_pair_register_frontend(
		...
		MAKE_CALLBACK(basic.ui->contextContainer->isVisible(),basic.HideContextBar, "Hiding Context Bar"),this, this);
...
#undef MAKE_CALLBACK
	...
	auto togglePreviewProgram = [](void *data, obs_hotkey_id,obs_hotkey_t *, bool pressed) {
		if (pressed)
			QMetaObject::invokeMethod(static_cast<OBSBasic *>(data), "on_modeSwitch_clicked",Qt::QueuedConnection);
	};

	togglePreviewProgramHotkey = obs_hotkey_register_frontend("OBSBasic.TogglePreviewProgram",Str("Basic.TogglePreviewProgramMode"), togglePreviewProgram,this);
	...
	auto transition = [](void *data, obs_hotkey_id, obs_hotkey_t *,bool pressed) {
		if (pressed)
			QMetaObject::invokeMethod(static_cast<OBSBasic *>(data),"TransitionClicked",Qt::QueuedConnection);
	};
	...
	auto resetStats = [](void *data, obs_hotkey_id, obs_hotkey_t *,bool pressed) {
		if (pressed)
			QMetaObject::invokeMethod(static_cast<OBSBasic *>(data),"ResetStatsHotkey",Qt::QueuedConnection);
	};

	statsHotkey = obs_hotkey_register_frontend("OBSBasic.ResetStats", Str("Basic.Stats.ResetStats"),resetStats, this);
	...
	auto screenshotSource = [](void *data, obs_hotkey_id, obs_hotkey_t *, bool pressed) {
		if (pressed)
			QMetaObject::invokeMethod(static_cast<OBSBasic *>(data),"ScreenshotSelectedSource",Qt::QueuedConnection);
	};

	sourceScreenshotHotkey = obs_hotkey_register_frontend("OBSBasic.SelectedSourceScreenshot",Str("Screenshot.SourceHotkey"), screenshotSource, this);
...
void OBSBasic::AddScene(OBSSource source)
{
	...
	obs_hotkey_register_source(source, "OBSBasic.SelectScene",Str("Basic.Hotkeys.SelectScene"),
...
void OBSBasic::UpdateSceneSelection(OBSSource source)
{
	if (source) {
		...
		QList<QListWidgetItem *> items = ui->scenes->findItems(QT_UTF8(name), Qt::MatchExactly);
...
void OBSBasic::MixerRenameSource()
{
	...
	for (;;) {
		...
		bool accepted = NameDialog::AskForName(
			this, QTStr("Basic.Main.MixerRename.Title"),QTStr("Basic.Main.MixerRename.Text"), name,	QT_UTF8(prevName));
		if (!accepted)
			return;
...
void OBSBasic::VolControlContextMenu()
{
	...
	toggleControlLayoutAction.setChecked(config_get_bool(GetGlobalConfig(), "BasicWindow", "VerticalVolControl"));
	...
	connect(&pasteFiltersAction, &QAction::triggered, this,&OBSBasic::AudioMixerPasteFilters, Qt::DirectConnection);
	connect(&filtersAction, &QAction::triggered, this,&OBSBasic::GetAudioSourceFilters, Qt::DirectConnection);
	connect(&propertiesAction, &QAction::triggered, this,&OBSBasic::GetAudioSourceProperties, Qt::DirectConnection);
	connect(&advPropAction, &QAction::triggered, this,&OBSBasic::on_actionAdvAudioProperties_triggered,Qt::DirectConnection);

	/* ------------------- */

	connect(&toggleControlLayoutAction, &QAction::changed, this,&OBSBasic::ToggleVolControlLayout, Qt::DirectConnection);
	...
	for (;;) {
		...
		if (name.empty()) {
			OBSMessageBox::warning(this,
					       QTStr("NoNameEntered.Title"),QTStr("NoNameEntered.Text"));
			continue;
		}
...
void OBSBasic::SourceCreated(void *data, calldata_t *params)
{
	...
	if (obs_scene_from_source(source) != NULL)
		QMetaObject::invokeMethod(static_cast<OBSBasic *>(data), "AddScene", WaitConnection(),Q_ARG(OBSSource, OBSSource(source)));
}
...
void OBSBasic::SourceRemoved(void *data, calldata_t *params)
{
	...
	if (obs_scene_from_source(source) != NULL)
		QMetaObject::invokeMethod(static_cast<OBSBasic *>(data),"RemoveScene",Q_ARG(OBSSource, OBSSource(source)));
}
...
void OBSBasic::SourceActivated(void *data, calldata_t *params)
{
	...
	if (flags & OBS_SOURCE_AUDIO)
		QMetaObject::invokeMethod(static_cast<OBSBasic *>(data),"ActivateAudioSource",Q_ARG(OBSSource, OBSSource(source)));
}
...
void OBSBasic::SourceDeactivated(void *data, calldata_t *params)
{
	...
	if (flags & OBS_SOURCE_AUDIO)
		QMetaObject::invokeMethod(static_cast<OBSBasic *>(data),"DeactivateAudioSource",Q_ARG(OBSSource, OBSSource(source)));
}
...
void OBSBasic::SourceRenamed(void *data, calldata_t *params)
{
	...
	QMetaObject::invokeMethod(static_cast<OBSBasic *>(data),"RenameSources", Q_ARG(OBSSource, source),Q_ARG(QString, QT_UTF8(newName)),Q_ARG(QString, QT_UTF8(prevName)));
...
int OBSBasic::ResetVideo()
{
	...
	ovi.colorspace = astrcmpi(colorSpace, "601") == 0 ? VIDEO_CS_601 : (astrcmpi(colorSpace, "709") == 0   ? VIDEO_CS_709 : VIDEO_CS_SRGB);
...
void OBSBasic::on_scenes_customContextMenuRequested(const QPoint &pos)
{
	...
	if (item) {
		...
		popup.addSeparator();
		// 复制选定的场景
		popup.addAction(QTStr("Duplicate"), this,SLOT(DuplicateSelectedScene()));
		// 拷贝场景滤镜
		...
		// 移除场景
		popup.addAction(QTStr("Remove"), this,SLOT(RemoveSelectedScene()));

		// 添加分割条
		popup.addSeparator();

		// 场景上移
		order.addAction(QTStr("Basic.MainMenu.Edit.Order.MoveUp"), this,SLOT(on_actionSceneUp_triggered()));
		// 场景下移
		order.addAction(QTStr("Basic.MainMenu.Edit.Order.MoveDown"),this, SLOT(on_actionSceneDown_triggered()));
		order.addSeparator();
		// 场景置顶
		order.addAction(QTStr("Basic.MainMenu.Edit.Order.MoveToTop"),this, SLOT(MoveSceneToTop()));
		// 场景置底
		order.addAction(QTStr("Basic.MainMenu.Edit.Order.MoveToBottom"),this, SLOT(MoveSceneToBottom()));
		popup.addMenu(&order);
		...
		// 场景预览(窗口)
		QAction *sceneWindow = popup.addAction(QTStr("SceneWindow"), this, SLOT(OpenSceneWindow()));

		popup.addAction(sceneWindow);
		// 场景截图
		popup.addAction(QTStr("Screenshot.Scene"), this,SLOT(ScreenshotScene()));
		popup.addSeparator();
		// 场景滤镜
		...
		connect(multiviewAction, &QAction::triggered,std::bind(showInMultiview, data));
	}
	// 添加分割条
	popup.addSeparator();
	...
	connect(gridAction, SIGNAL(triggered()), this,SLOT(on_actionGridMode_triggered()));
...
void OBSBasic::CreateSourcePopupMenu(int idx, bool preview)
{
	...
	if (preview) {
		...
		// 截屏(预览)
		popup.addAction(QTStr("Screenshot.Preview"), this,SLOT(ScreenshotScene()));
		popup.addSeparator();
	}
	...
	if (idx != -1) {
		...
		//截屏
		popup.addAction(QTStr("Screenshot.Source"), this,SLOT(ScreenshotSelectedSource()));
		popup.addSeparator();
...
void OBSBasic::AddSource(const char *id)
{
	if (id && *id) {
		...
		if (sourceSelect.newSource && strcmp(id, "group") != 0)
			CreatePropertiesWindow(sourceSelect.newSource);// 创建对应源的属性菜单
	}
}
//创建添加源弹出菜单
QMenu *OBSBasic::CreateAddSourcePopupMenu()
{
	...
	auto addSource = [this, getActionAfter](QMenu *popup, const char *type,
		...
		connect(popupItem, SIGNAL(triggered(bool)), this,SLOT(AddSourceFromAction()));// 为每个菜单项添加事件处理
...
void OBSBasic::on_streamButton_clicked()
{
	if (outputHandler->StreamingActive()) {
		...
		if (confirm && isVisible()) {
			QMessageBox::StandardButton button =OBSMessageBox::question(this, QTStr("ConfirmStop.Title"),QTStr("ConfirmStop.Text"),	QMessageBox::Yes | QMessageBox::No,	QMessageBox::No);
...
void OBSBasic::SystemTrayInit()
{
	...
	sysTrayVirtualCam = new QAction(QTStr("Basic.Main.StartVirtualCam"),trayIcon.data());
	...
	connect(sysTrayVirtualCam.data(), &QAction::triggered, this,&OBSBasic::VCamButtonClicked);
