...
int AutoUpdateThread::queryUpdate(bool localManualUpdate, const char *text_utf8)
{
	...
	QMetaObject::invokeMethod(this, "queryUpdateSlot", Qt::BlockingQueuedConnection, Q_RETURN_ARG(int, ret), Q_ARG(bool, localManualUpdate), Q_ARG(QString, text));
	return ret;
}
...
try {
	...
	success = GetRemoteFile(WIN_MANIFEST_URL, text, error, &responseCode,nullptr, nullptr, extraHeaders, &signature);
	...
	BPtr<char> updateFilePath = GetConfigPathPtr("obs-studio\\updates\\updater.exe");
