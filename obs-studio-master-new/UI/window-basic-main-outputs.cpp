...
{
	if (main->vcamEnabled) {
		obs_output_set_media(virtualCam, obs_get_video(), obs_get_audio());
		if (!Active())
			SetupOutputs();
...
SimpleOutput::SimpleOutput(OBSBasic *main_) : BasicOutputHandler(main_)
{
	...
	if (!CreateAACEncoder(aacArchive, aacArchiveEncID, GetAudioBitrate(),SIMPLE_ARCHIVE_NAME, 1))
...
AdvancedOutput::AdvancedOutput(OBSBasic *main_) : BasicOutputHandler(main_)
{
	...
	if (!CreateAACEncoder(streamAudioEnc, id, GetAudioBitrate(streamTrack),"adv_stream_aac", streamTrack))
	...
	if (!CreateAACEncoder(streamArchiveEnc, id, GetAudioBitrate(vodTrack),ADV_ARCHIVE_NAME, vodTrack))
