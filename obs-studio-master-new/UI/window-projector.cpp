...
OBSProjector::OBSProjector(QWidget *widget, obs_source_t *source_, int monitor,ProjectorType type_)
	: OBSQTDisplay(widget, Qt::Window),
	  source(source_),
	  removedSignal(obs_source_get_signal_handler(source), "remove",OBSSourceRemoved, this)
{
	// 置顶
	isAlwaysOnTop = config_get_bool(GetGlobalConfig(), "BasicWindow","ProjectorAlwaysOnTop");
...
void OBSProjector::OBSRenderMultiview(void *data, uint32_t cx, uint32_t cy)
{
	...
	if (multiviewLayout == MultiviewLayout::HORIZONTAL_TOP_24_SCENES) {
		...
		paintAreaWithColor(window->thickness, window->thickness,
				   ...
				   backgroundColor);
		paintAreaWithColor(window->thickness +2.5 * (window->thicknessx2 + window->ppiCX),
			window->thickness, window->siCX,window->siCY * 2 + window->thicknessx2,
			backgroundColor);
	}
	endRegion();
}
