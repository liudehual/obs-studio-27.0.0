...
#define VLC_META(media, cd, did, tid, tag)                               \
	...
	VLC_META(media, cd, data_id, "artist", libvlc_meta_Artist)//艺术家
	VLC_META(media, cd, data_id, "genre", libvlc_meta_Genre)//流派
	VLC_META(media, cd, data_id, "copyright", libvlc_meta_Copyright)//版权
	VLC_META(media, cd, data_id, "album", libvlc_meta_Album)//专辑
	VLC_META(media, cd, data_id, "track_number", libvlc_meta_TrackNumber)//轨道编号
	VLC_META(media, cd, data_id, "description", libvlc_meta_Description)//描述
	VLC_META(media, cd, data_id, "rating", libvlc_meta_Rating)//评分
	VLC_META(media, cd, data_id, "date", libvlc_meta_Date)//日期
	VLC_META(media, cd, data_id, "setting", libvlc_meta_Setting)//设置
	VLC_META(media, cd, data_id, "url", libvlc_meta_URL)//地址
	VLC_META(media, cd, data_id, "language", libvlc_meta_Language)//语言
	VLC_META(media, cd, data_id, "now_playing", libvlc_meta_NowPlaying)//正在播放
	VLC_META(media, cd, data_id, "publisher", libvlc_meta_Publisher)//发行商
	VLC_META(media, cd, data_id, "encoded_by", libvlc_meta_EncodedBy)//编码者
	VLC_META(media, cd, data_id, "artwork_url", libvlc_meta_ArtworkURL)//专辑图片地址
	VLC_META(media, cd, data_id, "track_id", libvlc_meta_TrackID)//轨道ID
	VLC_META(media, cd, data_id, "track_total", libvlc_meta_TrackTotal)//轨道总数
	VLC_META(media, cd, data_id, "director", libvlc_meta_Director)//导演
	VLC_META(media, cd, data_id, "season", libvlc_meta_Season)//第几季
	VLC_META(media, cd, data_id, "episode", libvlc_meta_Episode)//插曲
	VLC_META(media, cd, data_id, "show_name", libvlc_meta_ShowName)//显示名
	VLC_META(media, cd, data_id, "actors", libvlc_meta_Actors)//演员名
	// 注释人：GCT,未找到定义
	//VLC_META(media, cd, data_id, "album_artist", libvlc_meta_AlbumArtist)// 专辑艺术家
	//VLC_META(media, cd, data_id, "disc_number", libvlc_meta_DiscNumber)//碟号
	//VLC_META(media, cd, data_id, "disc_total", libvlc_meta_DiscTotal)//总碟数
...
static void vlcs_update(void *data, obs_data_t *settings)
{
	...
	libvlc_media_list_player_set_playback_mode_(c->media_list_player, c->loop ? libvlc_playback_mode_loop : libvlc_playback_mode_default);
