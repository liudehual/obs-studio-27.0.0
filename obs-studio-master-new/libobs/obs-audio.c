		    ...
		    struct audio_output_data *mixes)
{
	...
	// 字节数 = 采样频率*声道数*量化位数/8(字节)
	size_t sample_rate = audio_output_get_sample_rate(audio->audio);// 采样频率
	size_t channels = audio_output_get_channels(audio->audio);// 声道数
	...
	for (size_t i = 0; i < audio->render_order.num; i++) {
		...
		if (audio->total_buffering_ticks == MAX_BUFFERING_TICKS && source->audio_ts < ts.start) {
			if (source->info.audio_render) {
				blog(LOG_DEBUG,"render audio source %s timestamp has gone backwards",obs_source_get_name(source));
