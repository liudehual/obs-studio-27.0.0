...
static uint64_t tick_sources(uint64_t cur_time, uint64_t last_time)
{
	...
	if (!last_time)
		last_time = cur_time - video_output_get_frame_time(obs->video.video);

	delta_time = cur_time - last_time;
	seconds = (float)((double)delta_time / 1000000000.0);

	// 执行截图等功能,lua脚本，python脚本等
	/* ------------------------------------- */
	/* call tick callbacks                   */
				...
				const bool gpu_active, int cur_texture)
{
	...
	// 将所有源数据渲染到video->render_texture(并非直接在QWidget上显示,仅仅将数据放到缓冲区内，后期一次性显示)
				     ...
				     struct video_data *input_frame, int count)
{
	...
	if (locked) {
		if (video->gpu_conversion) {
			set_gpu_converted_data(video, &output_frame,input_frame, info);
		} else {
...
static inline void output_frame(bool raw_active, const bool gpu_active)
{
	...
	// 刷新绘制区域
	// 将所有的视频源一次性刷入显示缓冲区，防止抖动
	gs_flush();

	// 离开绘制上下文
	gs_leave_context();

	// 帧数据已经准备好，输出帧数据到video->cache队列，用于编码推流
...
bool obs_graphics_thread_loop(struct obs_graphics_context *context)
{
	...
	//计算线程休眠时间
	video_sleep(&obs->video, raw_active, gpu_active, &obs->video.video_time, context->interval);
	...
	if (context->fps_total_ns >= 1000000000ULL) {
		obs->video.video_fps =	(double)context->fps_total_frames / ((double)context->fps_total_ns / 1000000000.0);
...
void *obs_graphics_thread(void *param)
{
	...
	const char *video_thread_name = profile_store_name(obs_get_profiler_name_store(),"obs_graphics_thread(%g" NBSP "ms)", interval / 1000000.);
