...
bool obs_reset_audio(const struct obs_audio_info *oai)
{
	...
	ai.name = "Audio";// 名字
	ai.samples_per_sec = oai->samples_per_sec;// 采样率
	ai.format = AUDIO_FORMAT_FLOAT_PLANAR;// 格式
	ai.speakers = oai->speakers;// 声道
	ai.input_callback = audio_callback;// 注册回调,该回调函数实际处理音频数据

	blog(LOG_INFO, "---------------------------------");
	blog(LOG_INFO,"audio settings reset:\n""\tsamples per sec: %d\n""\tspeakers:        %d",(int)ai.samples_per_sec, (int)ai.speakers);
...
static inline struct obs_modeless_ui *get_modeless_ui_callback(const char *id, const char *task, const char *target)
{
...
static void obs_render_main_texture_internal(enum gs_blend_type src_c,enum gs_blend_type dest_c,enum gs_blend_type src_a,enum gs_blend_type dest_a)
{
...
bool obs_context_data_init(struct obs_context_data *context,
			   ...
			   bool private)
{
	if (obs_context_data_init_wrap(context, type, settings, name,hotkey_data, private)) {
		return true;
	} else {
