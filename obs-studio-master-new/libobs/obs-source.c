...
struct obs_source_info *get_source_info(const char *id)
{
	for (size_t i = 0; i < obs->source_types.num; i++) {
		...
		if (info->id && strlen(info->id) && id && strlen(id) &&strcmp(info->id, id) == 0)
			return info;
	}
...
{
	if (!obs_context_data_init(&source->context, OBS_OBJ_TYPE_SOURCE,settings, name, hotkey_data, private))
		return false;
...
static obs_source_t *obs_source_create_internal(const char *id, const char *name,obs_data_t *settings, obs_data_t *hotkey_data,bool private, uint32_t last_obs_ver)
{
	...
	if (!private)
		obs_source_init_audio_hotkeys(source);// 添加了外部私有数据，初始化音频热键
	...
	if (!private) {
		// 添加了外部私有数据，调用外部注册的信号回调函数
		obs_source_dosignal(source, "source_create", NULL);
	}
	//source是音频源则加入到obs->data 音频源列表中
				   ...
				   gs_texrender_t *texrender)
{
	...
	if (success) {
		...
		vec4_set(&vec0, frame->color_matrix[0], frame->color_matrix[1], frame->color_matrix[2], frame->color_matrix[3]);
		vec4_set(&vec1, frame->color_matrix[4], frame->color_matrix[5], frame->color_matrix[6], frame->color_matrix[7]);
		vec4_set(&vec2, frame->color_matrix[8], frame->color_matrix[9], frame->color_matrix[10], frame->color_matrix[11]);
		...
		// 绘制原始帧数据(此处只是绘制，类似拷贝到显存，不显示，等待刷新显示)
		gs_draw(GS_TRIS, 0, 3);
			   ...
			   gs_texrender_t *texrender)
{
	...
	// 存在显卡，source->async_gpu_conversion 为true,使用显卡渲染
...
static inline void obs_source_render_async_video(obs_source_t *source)
{
	if (source->async_textures[0] && source->async_active) {
		...
		if (rotation) {
			gs_matrix_push();
			rotate_async_video(source, rotation); // 异步视频旋转,顺时针/逆时针
		}
...
static inline void obs_source_main_render(obs_source_t *source)
{
	...
	if (default_effect)
		...
		source->info.video_render(source->context.data,custom_draw ? NULL : gs_get_effect());
}
...
static inline void render_video(obs_source_t *source)
{
	...
	// 输入视频源
	if (source->info.type == OBS_SOURCE_TYPE_INPUT && (source->info.output_flags & OBS_SOURCE_ASYNC) != 0 &&  !source->rendering_filter) {
		if (deinterlacing_enabled(source))// 逐行扫描
	...
	GS_DEBUG_MARKER_BEGIN_FORMAT(GS_DEBUG_COLOR_SOURCE,get_type_format(source->info.type),obs_source_get_name(source));
	...
	else if (deinterlacing_enabled(source))//逐行扫描
		deinterlace_render(source);
	else
		obs_source_render_async_video(source);// 最后渲染异步视频
					    ...
					    struct obs_source_frame *in)
{
	...
	for (i = source->filters.num; i > 0; i--) {
		...
		if (filter->context.data && filter->info.filter_video) {
			in = filter->info.filter_video(filter->context.data,in);// 执行滤镜
			if (!in)
				break;
		}
	}
				 ...
				 const struct obs_source_frame *frame)
{
	...
	struct obs_source_frame *output = frame ? cache_video(source, frame) : NULL;
			       ...
			       size_t channels, size_t sample_rate)
{
	...
	if (vol == 0.0f || mixers == 0) {
		memset(source->audio_output_buf[0][0], 0, AUDIO_OUTPUT_FRAMES * sizeof(float) * MAX_AUDIO_CHANNELS * MAX_AUDIO_MIXES);
		return;
	}
