				  ...
				  struct audio_data *data)
{
	...
	if (input->resampler) {
		...
		success = audio_resampler_resample(input->resampler, output, &frames, &offset,(const uint8_t *const *)data->data, data->frames);

		for (size_t i = 0; i < MAX_AV_PLANES; i++)data->data[i] = output[i];
		data->frames = frames;
...
static void *audio_thread(void *param)
{
	...
	const char *audio_thread_name =	profile_store_name(obs_get_profiler_name_store(),"audio_thread(%s)", audio->info.name);
...
int audio_output_open(audio_t **audio, struct audio_output_info *info)
{
	...
	out->channels = get_audio_channels(info->speakers);//音频通道数
	out->planes = planar ? out->channels : 1;// 音频平面数
	out->input_cb = info->input_callback;// 音频数据输入回调
	out->input_param = info->input_param;// 音频数据输入参数
	out->block_size = (planar ? 1 : out->channels) * get_audio_bytes_per_channel(info->format);// 计算音频数据大小 通道数*格式

	if (pthread_mutexattr_init(&attr) != 0)
		goto fail;
	if (pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE) != 0)//允许同一个线程中可以多次获取同一把锁,并且不会死锁
		goto fail;
