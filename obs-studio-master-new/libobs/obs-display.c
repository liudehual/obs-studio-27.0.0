				  ...
				  uint32_t background_color)
{
	...
	bool ret = obs_display_init(display, graphics_data);
...
static inline void render_display_begin(struct obs_display *display,uint32_t cx, uint32_t cy,bool size_changed)
{
...
void render_display(struct obs_display *display)
{
	...
	/* -------------------------------------------- */
	// 将渲染子系统和对应QWidget句柄关联，方便后期渲染数据
	render_display_begin(display, cx, cy, size_changed);

	pthread_mutex_lock(&display->draw_callbacks_mutex);

	// 遍历所有obs_display注册的回调函数,并调用obs_display注册的回调函数进行一些坐标设置等操作
	for (size_t i = 0; i < display->draw_callbacks.num; i++) {
		struct draw_callback *callback = NULL;
	...
	// 一次性刷新所有数据到所有QWidget(主预览/属性预览/多视图预览等)
	gs_present();
}
